...
func get_possible_pickup_dropoff_locations() -> Array[Vector2]:
	...
	for tilemap_cell : Vector2i in all_tilemap_cells:
		var cell_data := get_cell_tile_data(0, tilemap_cell)
		var spawn_offset : Vector2 = cell_data.get_custom_data("spawn_offset")
		...
		if spawn_offset != Vector2.ZERO:
			...
			var cell_location : Vector2 = map_to_local(tilemap_cell)
